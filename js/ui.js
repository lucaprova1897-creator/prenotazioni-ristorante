...
const UI = {
  ...
  dateLabel(iso) {
    ...
    const tomorrow = Utils.dateToISO(new Date(Date.now() + 86400000));
    const yesterday = Utils.dateToISO(new Date(Date.now() - 86400000));
  ...
  buildBookingRow(b) {
    ...
    row.addEventListener('click', () => this.openEditForm(b.id));
    return row;
  },
  ...
  async saveForm() {
    ...
    if (this.state.editingBookingId) {
      const result = await Bookings.update(this.state.editingBookingId, data);
    ...
    if (synced === false) {
      this.toast('Salvata localmente (offline)', 'warning');
    } else {
      this.toast(this.state.editingBookingId ? 'Prenotazione aggiornata ✓' : 'Prenotazione salvata ✓');
    }
